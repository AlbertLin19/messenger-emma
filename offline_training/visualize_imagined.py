...
def evaluate(args):
    ...
    for split, dataloader in dataloaders.items():
        ...
        with torch.no_grad():
            ...
            z = 0
            with open(os.path.join(args.output_folder, f"{split}_gt.txt"), "a") as f:
                f.write(str(z) + "\n")
                f.write(str(tensor_grids[1].sum(-1)) + "\n")
            ...
            while True:
                ...
                with open(
                    os.path.join(args.output_folder, f"{split}_gt.txt"), "a"
                ) as f:
                    f.write(str(z) + "\n")
                    f.write(str(tensor_grids[1].sum(-1)) + "\n")
                ...
                with open(
                    os.path.join(args.output_folder, f"{split}_{args.manuals}.txt"), "a"
                ) as f:
                    f.write(str(z) + "\n")
                    f.write(str(tensor_grids[1].sum(-1)) + "\n")
...
if __name__ == "__main__":
    ...
    if args.output_folder is None:
        args.output_folder = f"visualization/imagined/{os.path.basename(args.load_model_from).split('.')[0]}/{args.manuals}/"
