...
for text_file in TEXT_FILES:
    ...
    chatgpt_grounding_paths = [
        os.path.join(SAVE_DIR, "chatgpt_grounding_for_" + text_file)
    ]
    while os.path.isfile(
        os.path.join(
            SAVE_DIR,
            f"refined_{len(chatgpt_grounding_paths)}_chatgpt_grounding_for_"
            + text_file,
        )
    ):
        chatgpt_grounding_paths.append(
            os.path.join(
                SAVE_DIR,
                f"refined_{len(chatgpt_grounding_paths)}_chatgpt_grounding_for_"
                + text_file,
            ...
            if len(chatgpt_grounding.keys()) == len(text_tuples):
                ...
                for i in range(len(text_tuples)):
                    ...
                    if movement_id == movement_id_gt:
                        n_mvmt_correct += 1
                print(
                    f"entities correct: {n_entity_correct}/{len(text_tuples)} = {n_entity_correct/len(text_tuples)}"
                )
                print(
                    f"mvmts correct: {n_mvmt_correct}/{len(text_tuples)} = {n_mvmt_correct/len(text_tuples)}"
                )
                print()
    print()
