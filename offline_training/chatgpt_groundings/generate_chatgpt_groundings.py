...
for text_file in TEXT_FILES:
    ...
    chatgpt_grounding_path = os.path.join(
        SAVE_DIR, "chatgpt_grounding_for_" + text_file
    )
    while os.path.isfile(
        os.path.join(
            SAVE_DIR, f"refined_{version+1}_chatgpt_grounding_for_" + text_file
        )
    ):
        chatgpt_grounding_path = os.path.join(
            SAVE_DIR, f"refined_{version+1}_chatgpt_grounding_for_" + text_file
    ...
    if os.path.isfile(chatgpt_grounding_path):
        with open(chatgpt_grounding_path, "r") as f:
            ...
            if len(chatgpt_grounding.keys()) == len(text_tuples):
                ...
                if version > 1:
                    old_chatgpt_grounding_path = os.path.join(
                        SAVE_DIR,
                        f"refined_{version-1}_chatgpt_grounding_for_" + text_file,
                    )
                else:
                    old_chatgpt_grounding_path = os.path.join(
                        SAVE_DIR, "chatgpt_grounding_for_" + text_file
    ...
    for i in tqdm(range(len(text_tuples))):
        ...
        if text in chatgpt_grounding:
            ...
            version > 0
            and old_chatgpt_grounding[text][0] >= 0
            and old_chatgpt_grounding[text][1] >= 0
        ):
            ...
            entity_id, movement_id = (
                (response.strip() + ",").replace(" ", "").split(",")[:2]
            )
