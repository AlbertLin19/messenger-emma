...
def collect_rollouts(env, n_sets_total, max_rollout_length):
    manuals = []
    ground_truths = []
    grid_sequences = []
    action_sequences = []
    pbar = tqdm(total = n_sets_total * len(env.all_games) * len(env.game_variants) * len(env.init_states))
    n_sets = 0
    while True:
