...
def collect_rollouts(env):
    ...
    while True:
        ...
        while not done:
            obs, reward, done, _ = env.step(random.choice(range(5)))
            obs = wrap_obs(obs)
        ...
        if (env.next_game_idx == 0) and (env.next_variant_idx == 0) and (env.next_init_state_idx == 0):
            break
    pbar.close()
    return manuals, ground_truths, grid_sequences

train_all_manuals, train_all_ground_truths, train_all_grid_sequences = collect_rollouts(train_all_env)
val_same_worlds_manuals, val_same_worlds_ground_truths, val_same_worlds_grid_sequences = collect_rollouts(val_same_worlds_env)
test_same_worlds_se_manuals, test_same_worlds_se_ground_truths, test_same_worlds_se_grid_sequences = collect_rollouts(test_same_worlds_se_env)
