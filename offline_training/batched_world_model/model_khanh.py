...
class WorldModelBase(nn.Module):
    ...
    def id_loss(self, logits, targets):
        ...
        loss = (
            F.cross_entropy(logits, targets, ignore_index=-1, reduction="sum")
            / normalizer
        )
    ...
    def reset(self, init_grids, idxs=None):
        if idxs is None:
            ...
            self.init_id_per_channel = init_grids.flatten(1, 2).max(1)[0]
            self.cnt_manual_diff = []
        else:
            self.hidden_states[:, idxs] = 0
            self.cell_states[:, idxs] = 0
            self.init_id_per_channel[idxs] = init_grids[idxs].flatten(1, 2).max(1)[0]
    ...
    def predict_grid(self, preds, sample=False):
        grids = torch.zeros((preds["loc"].shape[0], H, W, GRID_CHANNELS)).to(
            self.device
        )
        if sample:
...
class WorldModel(WorldModelBase):
    def __init__(self, args):
        ...
        self.keep_entity_features_for_parsed_manuals = (
            args.keep_entity_features_for_parsed_manuals
        )
        ...
        if args.manuals == "embed":
            ...
            self.token_key_att = nn.Sequential(
                nn.Linear(768, 384), nn.ReLU(), nn.Linear(384, 1), nn.Softmax(dim=-2)
            )
        ...
        self.before_lstm_projector = nn.Sequential(
            nn.Flatten(), nn.Linear(enc_dim, hidden_size), nn.ReLU()
        )
        ...
        self.after_lstm_projector = nn.Sequential(
            nn.Linear(hidden_size, enc_dim), nn.ReLU()
        )
        ...
        nonexistence_layers = [nn.Linear(hidden_size, hidden_size), nn.ReLU()]
        for _ in range(self.encoder.num_layers - 1):
            nonexistence_layers.extend([nn.Linear(hidden_size, hidden_size), nn.ReLU()])
        ...
        # loss accumulation
        self.loss = {"loc": 0, "id": 0, "reward": 0, "done": 0}
    ...
    def embed_grids_with_parsed_manuals(self, grids, parsed_manuals):
        ...
        for triplet in parsed_manuals:
            movements.append(
                [MOVEMENT_TYPES[e[1]] if e is not None else 4 for e in triplet] + [3]
            )
            roles.append(
                [ROLE_TYPES[e[2]] if e is not None else 4 for e in triplet] + [3]
            )
    ...
    def embed_grids_with_embedded_manuals(self, grids, embedded_manuals):
        ...
        entity_query_embed = self._select(
            self.entity_query_embeddings, grids
        )  # b x h x w x c x desc_key_dim
        desc_key = torch.sum(
            self.token_key_att(embedded_manuals) * self.token_key(embedded_manuals),
            dim=-2,
        )  # b x 3 (num_sent) x desc_key_dim
        desc_att_logits = torch.matmul(
            entity_query_embed,
            desc_key.permute(0, 2, 1).view(b, 1, 1, self.desc_key_dim, 3),
        )  # b x h x w x c x 3 (num_sent)
        desc_att = F.softmax(desc_att_logits / np.sqrt(self.desc_key_dim), dim=-1)
        # compute description movement/role values
        desc_movement_val = torch.sum(
            self.token_movement_val_att(embedded_manuals)
            * self.token_movement_val(embedded_manuals),
            dim=-2,
        )  # b x 3 (num_sent) x attr_embed_dim
        desc_role_val = torch.sum(
            self.token_role_val_att(embedded_manuals)
            * self.token_role_val(embedded_manuals),
            dim=-2,
        )  # b x 3 (num_sent) x attr_embed_dim
        # compute movements_embed and roles_embed
        movements_embed = torch.sum(
            desc_att.view(b, h, w, c, 3, 1)
            * desc_movement_val.view(b, 1, 1, 1, 3, self.attr_embed_dim),
            dim=-2,
        )  # b x h x w x c x attr_embed_dim
        roles_embed = torch.sum(
            desc_att.view(b, h, w, c, 3, 1)
            * desc_role_val.view(b, 1, 1, 1, 3, self.attr_embed_dim),
            dim=-2,
        )  # b x h x w x c x attr_embed_dim
        ...
        movements_embed[..., -1, :] = self.movement_embeddings(
            torch.tensor([3], device=self.device)
        )
        roles_embed[..., -1, :] = self.role_embeddings(
            torch.tensor([3], device=self.device)
        )
    ...
    def forward(
        self, grids, rewards, embedded_manuals, parsed_manuals, actions, lstm_states
    ):
        if self.manuals in ["gpt", "oracle"]:
            grids_embed = self.embed_grids_with_parsed_manuals(grids, parsed_manuals)
        elif self.manuals == "embed":
            grids_embed = self.embed_grids_with_embedded_manuals(
                grids, embedded_manuals
            )
        ...
        logits["id"] = logits["id"].view(
            logits["id"].shape[0], GRID_CHANNELS, NUM_ENTITIES
        )
    ...
    def create_loc_logits_and_targets(self, grid_logits, nonexistence_logits, grids):
        ...
        grid_targets = grid_targets.view(
            grid_targets.shape[0], grid_targets.shape[1], -1
        )
        nonexistence_targets = (1.0 - grid_targets.sum(dim=-1)).unsqueeze(-1)
        location_targets = torch.cat((grid_targets, nonexistence_targets), dim=-1)

        return location_logits, location_targets

    def create_id_logits_and_targets(self, id_logits, grids):
        id_targets = grids.flatten(1, 2).max(1)[0]
        # for gpt and oracle, only predict id of agent
        if self.manuals in ["gpt", "oracle"]:
            id_targets[:, :3] = -1
        return id_logits, id_targets

    def reorder_parsed_manuals(self, parsed_manuals, grids):
        curr_id_per_channel = grids.flatten(1, 2).max(1)[0]
        new_parsed_manuals = []
        for i, triplet in enumerate(parsed_manuals):
            new_triplet = []
            for j, r in enumerate(self.init_id_per_channel[i, :-1].tolist()):
                new_triplet.append(
    ...
    def reward_to_label(self, r):
        # float from -1 to 1 ---> integer from 0 to 4
        return ((r + 1) * 2).long()
    ...
    def step(
        ...
        true_parsed_manuals = self.reorder_parsed_manuals(true_parsed_manuals, old_grids)
        ...
        logits["loc"], targets["loc"] = self.create_loc_logits_and_targets(
            logits["grid"], logits["nonexistence"], targets["grid"]
        )
        logits["id"], targets["id"] = self.create_id_logits_and_targets(
            logits["id"], grids[backprop_idxs]
        )
    ...
    """
    def pred(self, old_grids, parsed_manuals, actions, sample):
        if self.manuals != "gpt":
            raise NotImplementedError
        ...
        logits["loc"] = self.create_loc_logits(logits["grid"], logits["nonexistence"])
