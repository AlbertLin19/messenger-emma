...
def batched_convert_grid_to_multilabel(grids):
    ...
    multilabels[..., 0] = (
        torch.sum(grids, dim=-1) < 1
    )  # the empty class # NOTE: this channel should be entirely ignored when using prediction_type = location
    return multilabels
...
def batched_ground(manuals, ground_truths, world_model):
    query = world_model.sprite_emb(
        torch.arange(17, device=world_model.device)
    )  # 17 x key_dim
    if world_model.key_type == "oracle":
        keys = F.one_hot(
            torch.tensor(
                [
                    [ENTITY_IDS[truth[0]] for truth in ground_truth]
...
def batched_convert_multilabel_to_emb(multilabels, manuals, ground_truths, world_model):
    if world_model.val_type == "oracle":
        ...
        values = manuals.shape[1] * F.one_hot(
            torch.tensor(
                [
                    [MOVEMENT_TYPES[truth[1]] for truth in ground_truth]
                    for ground_truth in ground_truths
                ],
                device=world_model.device,
            ),
            num_classes=world_model.val_dim,
        )
        values += manuals.shape[1] * F.one_hot(
            torch.tensor(
                [
                    [ROLE_TYPES[truth[2]] for truth in ground_truth]
                    for ground_truth in ground_truths
                ],
                device=world_model.device,
            ),
    ...
    entity_values = torch.mean(
        weights.unsqueeze(-1) * values.unsqueeze(-3), dim=-2
    )  # B x 17 x val_dim
    ...
    entity_values = (
        entity_values.unsqueeze(-3).unsqueeze(-3) * multilabels[..., None]
    )  # B x 10 x 10 x 17 x val_dim
    entity_values = torch.sum(entity_values, dim=-2)  # B x 10 x 10 x val_dim
    return torch.cat(
        (multilabels[..., world_model.relevant_cls_idxs], entity_values), dim=-1
    )
...
def batched_convert_prob_to_multilabel(
    probs, nonexistence_probs, prediction_type, threshold, refine, entity_ids
):
    ...
    if refine:
        multilabels = multilabels * (
            probs >= torch.amax(probs, dim=(1, 2), keepdim=True)
        )
        multilabels[..., 15:17] = probs[..., 15:17] >= torch.amax(
            probs[..., 15:17], dim=(1, 2, 3), keepdim=True
        )
        multilabels[..., :15] = (
            multilabels[..., :15]
            * (F.one_hot(entity_ids, num_classes=15).sum(dim=-2))[:, None, None, :]
        )
