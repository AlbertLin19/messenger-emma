...
def get_manual(dataset, split, i):
    ...
    return [
        texts[entities[ground_truth_idx[j][0]]][dynamics[ground_truth_idx[j][1]]][
...
def get_ground_truth(dataset, split, i):
    ...
    return [
        (
            CUSTOM_TO_MESSENGER_ENTITY[entities[ground_truth_idx[j][0]]],
            CUSTOM_TO_MESSENGER_DYNAMIC[dynamics[ground_truth_idx[j][1]]],
            CUSTOM_TO_MESSENGER_ROLE[roles[ground_truth_idx[j][2]]],
        )
...
class DataLoader:
    def __init__(
        ...
        self.n_rollouts = min(
            len(dataset["rollouts"][split]["manual_idxs"]), max_rollouts
        )
        self.rollout_lengths = np.array(
            [
                min(len(sequence), max_rollout_length)
                for sequence in dataset["rollouts"][split]["grid_sequences"][
                    : self.n_rollouts
                ]
            ]
        )
        self.rollout_probs = (self.rollout_lengths - 1) / np.sum(
            self.rollout_lengths - 1
        )
        ...
        self.action_sequences_array = np.zeros(
            (self.n_rollouts, np.max(self.rollout_lengths)), dtype=int
        )
        self.reward_sequences_array = np.zeros(
            (self.n_rollouts, np.max(self.rollout_lengths)), dtype=float
        )
        self.done_sequences_array = np.zeros(
            (self.n_rollouts, np.max(self.rollout_lengths)), dtype=bool
        )
        self.grid_sequences_array = np.zeros(
            (self.n_rollouts, np.max(self.rollout_lengths), 10, 10, 4), dtype=int
        )
        for i in tqdm(range(self.n_rollouts)):
            ...
            self.action_sequences_array[i, : self.rollout_lengths[i]] = dataset[
                "rollouts"
            ][split]["action_sequences"][i][:max_rollout_length]
            self.reward_sequences_array[i, : self.rollout_lengths[i]] = dataset[
                "rollouts"
            ][split]["reward_sequences"][i][:max_rollout_length]
            self.done_sequences_array[i, : self.rollout_lengths[i]] = dataset[
                "rollouts"
            ][split]["done_sequences"][i][:max_rollout_length]
            self.grid_sequences_array[i, : self.rollout_lengths[i]] = dataset[
                "rollouts"
            ][split]["grid_sequences"][i][:max_rollout_length]
        ...
        if self.mode == "random":
            if self.start_state == "initial":
                ...
                self.indices = np.random.choice(
                    self.n_rollouts, size=self.batch_size, p=self.rollout_probs
                )
                # randomly sample starting state for each rollout
                self.timesteps = (
                    np.random.rand(self.batch_size)
                    * (self.rollout_lengths[self.indices] - 1)
                ).astype(int)
            else:
    ...
    def step(self):
        if self.mode == "random":
            ...
            new_idxs = np.argwhere(
                self.timesteps >= self.rollout_lengths[self.indices]
            ).squeeze(-1)
            cur_idxs = np.argwhere(
                self.timesteps < self.rollout_lengths[self.indices]
            ).squeeze(-1)
            if self.start_state == "initial":
                # randomly sample batch_size rollouts from data (with equal probability) and keep track of their lengths
                self.indices[new_idxs] = np.random.choice(
                    self.n_rollouts, size=len(new_idxs)
                )
                # start at initial state for each rollout
                ...
                # randomly sample batch_size rollouts from data (with probability proportional to their lengths) and keep track of their lengths
                self.indices[new_idxs] = np.random.choice(
                    self.n_rollouts, size=len(new_idxs), p=self.rollout_probs
                )
                self.timesteps[new_idxs] = (
                    np.random.rand(len(new_idxs))
                    * (self.rollout_lengths[self.indices[new_idxs]] - 1)
                ).astype(int)
            else:
                raise NotImplementedError
            ...
            new_idxs = np.argwhere(
                (self.indices >= 0)
                * (self.timesteps >= self.rollout_lengths[nonnegative_indices])
            ).squeeze(-1)
            cur_idxs = np.argwhere(
                (self.indices >= 0)
                * (self.timesteps < self.rollout_lengths[nonnegative_indices])
            ...
            return (
                ...
                (self.indices >= 0)
                * (
                    self.timesteps == (self.rollout_lengths[nonnegative_indices] - 1)
                ),  # whether rollout was just completed
