...
for subfolder in subfolders:
    for split in os.listdir(os.path.join(folder, subfolder)):
        ...
        for model in ['none', 'oracle', 'gpt', 'embed']:
            with open(os.path.join(folder, subfolder, split, model, split.replace('dev', 'test').replace('best_loc_loss', 'loc_perps')+'.json'), 'r') as f:
                perps = json.load(f)
                ax.plot(range(1, len(perps)+1), perps, c=MODEL_COLORS[model], label=MODEL_NAMES[model])
