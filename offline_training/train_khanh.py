...
from dataloader import DataLoader, get_manual, get_ground_truth
...
def train(args):
    ...
    """
    print(len(gpt_groundings))
    cnt = []
    manual_to_parse = {}
    for i in range(len(dataset["rollouts"]["train_games"]["manual_idxs"])):
        manual = get_manual(dataset, "train_games", i)
        gpt_manual = [gpt_groundings[e] for e in manual]
        gt_manual = get_ground_truth(dataset, "train_games", i)
        for x, y, z in zip(manual, gpt_manual, gt_manual):
            manual_to_parse[x] = (y, z)
        cnt.append(gpt_manual == gt_manual)
    print(np.average(cnt))
    cnt = []
    for k, v in manual_to_parse.items():
        cnt.append(v[0] == v[1])
    print(np.average(cnt))
    input()
    """
    ...
    # reset world model
    world_model.reset(tensor_grids)
    ...
    while step < args.max_step:
        ...
        world_model.reset(tensor_grids, new_idxs)
...
def evaluate(
    ...
    avg_metric["total_loss"] = (
        avg_metric["loc_loss"]
        + avg_metric["id_loss"]
        + avg_metric["reward_loss"]
        + avg_metric["done_loss"]
    )
    ...
    if args.manuals == "embed":
        avg_metric["grounding"] = evaluate_grounding(
            args, world_model, manuals_encoder, dataloader
        )
    """
...
def evaluate_grounding(args, world_model, manuals_encoder, dataloader):
        ...
        for j in range(dataloader.n_rollouts):
            ...
            for k in range(len(dataloader.ground_truths_array[j])):
                if ENTITY_IDS[dataloader.ground_truths_array[j][k][0]] == entity_ids[i]:
                    embedded_manual[i], _ = manuals_encoder.encode(
                        [[dataloader.manuals_array[j][k]]]
                    )
                    found = True
                    break
        if not found:
    ...
    # compute grounding
    entity_query = world_model.entity_query_embeddings(
        torch.tensor(entity_ids).to(args.device)
    )  # 12 x key_dim
    desc_key = torch.sum(
        world_model.token_key_att(embedded_manual)
        * world_model.token_key(embedded_manual),
        dim=-2,
    )  # 12 x key_dim
    desc_att_logits = torch.mm(
        entity_query, desc_key.T
    )  # 12 (entities) x 12 (desc) grounding
...
def add_eval_metrics(metrics, preds, targets, timesteps):
    ...
    metrics["done_loss"].extend(
        F.binary_cross_entropy(preds["done"], targets["done"], reduction="none")
        .view(-1)
        .tolist()
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--load_model_from", default=None, help="Path to world model state dict."
    )
    parser.add_argument(
        "--hidden_size", default=1024, type=int, help="World model hidden size."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--learning_rate", default=0.0001, type=float, help="World model learning rate."
    )
    parser.add_argument(
        "--weight_decay", default=0, type=float, help="World model weight decay."
    )
    parser.add_argument(
        "--reward_loss_weight",
        default=1,
    ...
    parser.add_argument("--max_step", default=500000, type=int, help="max training step")
    ...
    parser.add_argument(
        "--eval_batch_size", default=32, type=int, help="batch_size for evaluation"
    )
    parser.add_argument(
    ...
    if args.use_wandb:
        wandb.init(
            project="messenger",
            entity=args.entity,
            group=args.manuals + "_50k_hidden_1024",
