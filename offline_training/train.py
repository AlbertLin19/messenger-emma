...
def train(args):
    ...
    eval_metric_mins = [
        f"eval_{eval_split}_real_nontrivial_grid_perplexity" for eval_split in splits
    ]
    eval_metric_mins.extend(
        [
            f"eval_{eval_split}_real_nontrivial_entity_grid_perplexity"
            for eval_split in splits
        ]
    )
    eval_metric_mins.extend(
        [f"eval_{eval_split}_real_nontrivial_reward_mse" for eval_split in splits]
    )
    eval_metric_mins.extend(
        [f"eval_{eval_split}_real_nontrivial_done_perplexity" for eval_split in splits]
    )
    ...
    while step < args.max_step:
        ...
        if args.world_model_key_freeze and (
            (args.world_model_key_unfreeze_step >= 0)
            and (step >= args.world_model_key_unfreeze_step)
        ):
            world_model.unfreeze_key()
            args.world_model_key_freeze = False
        if args.world_model_val_freeze and (
            (args.world_model_val_unfreeze_step >= 0)
            and (step >= args.world_model_val_unfreeze_step)
        ):
        ...
        if step % args.update_step == 0:
            ...
            (
                ...
                real_done_loss,
                real_loss,
            ) = world_model.real_loss_reset()
            (
                imag_grid_loss,
                imag_reward_loss,
                imag_done_loss,
                imag_loss,
            ) = world_model.imag_loss_reset()
        ...
        if step % args.eval_step == 0:
            with torch.no_grad():
                ...
                for eval_split, eval_dataloader in eval_dataloaders.items():
                    ...
                    # create evaluators
                    eval_real_evaluator = Evaluator(
                        eval_world_model,
                        f"eval_{eval_split}_real_",
                        args.max_rollout_length,
                        eval_world_model.relevant_cls_idxs,
                        args.n_frames,
                        args.device,
                    )
                    eval_imag_evaluator = Evaluator(
                        eval_world_model,
                        f"eval_{eval_split}_imag_",
                        args.max_rollout_length,
                        eval_world_model.relevant_cls_idxs,
                        args.n_frames,
                        args.device,
                    ...
                    eval_tensor_grids = (
                        torch.from_numpy(eval_grids).long().to(args.device)
                    )
                    ...
                    while True:
                        ...
                        (
                            ...
                            eval_rewards,
                            eval_dones,
                            (eval_new_idxs, eval_cur_idxs),
                            eval_timesteps,
                            eval_just_completes,
                            eval_all_complete,
                        ) = eval_dataloader.step()
                        if eval_all_complete:
                            break
                        eval_manuals, eval_tokens = encoder.encode(eval_manuals)
                        eval_tensor_actions = (
                            torch.from_numpy(eval_actions).long().to(args.device)
                        )
                        eval_tensor_grids = (
                            torch.from_numpy(eval_grids).long().to(args.device)
                        )
                        eval_tensor_rewards = (
                            torch.from_numpy(eval_rewards).float().to(args.device)
                        )
                        eval_tensor_dones = (
                            torch.from_numpy(eval_dones).long().to(args.device)
                        )
                        eval_tensor_timesteps = (
                            torch.from_numpy(eval_timesteps).long().to(args.device)
                        )

                        # predict
                        eval_real_results = eval_world_model.real_step(
                            eval_old_tensor_grids,
                            eval_manuals,
                            eval_ground_truths,
                            eval_tensor_actions,
                            eval_tensor_grids,
                        ...
                        eval_imag_results = eval_world_model.imag_step(
                            eval_manuals,
                            eval_ground_truths,
                            eval_tensor_actions,
                            eval_tensor_grids,
                        ...
                        # evaluate
                        eval_real_evaluator.push(
                            eval_real_results,
                            (eval_manuals, eval_tokens),
                            eval_ground_truths,
                            (eval_new_idxs, eval_cur_idxs),
                            eval_world_model.real_entity_ids,
                            eval_tensor_timesteps,
                        )
                        eval_imag_evaluator.push(
                            eval_imag_results,
                            (eval_manuals, eval_tokens),
                            eval_ground_truths,
                            (eval_new_idxs, eval_cur_idxs),
                            eval_world_model.imag_entity_ids,
                            eval_tensor_timesteps,
                        )

                        # reset eval_world_model hidden states for new rollouts
                        eval_world_model.real_state_reset(
                            eval_tensor_grids, eval_new_idxs
                        )
                        eval_world_model.imag_state_reset(
                            eval_tensor_grids, eval_new_idxs
                        )
                    ...
                    (
                        eval_real_grid_loss,
                        eval_real_reward_loss,
                        eval_real_done_loss,
                        eval_real_loss,
                    ) = eval_world_model.real_loss_reset()
                    (
                        eval_imag_grid_loss,
                        eval_imag_reward_loss,
                        eval_imag_done_loss,
                        eval_imag_loss,
                    ) = eval_world_model.imag_loss_reset()
                    eval_updatelog.update(
                        {
                            ...
                            f"eval_{eval_split}_real_grid_loss_perplexity": np.exp(
                                eval_real_grid_loss
                            ),
                            f"eval_{eval_split}_imag_grid_loss_perplexity": np.exp(
                                eval_imag_grid_loss
                            ),
                            f"eval_{eval_split}_real_done_loss_perplexity": np.exp(
                                eval_real_done_loss
                            ),
                            f"eval_{eval_split}_imag_done_loss_perplexity": np.exp(
                                eval_imag_done_loss
                            ),
                        }
                    )
                ...
                for metric in eval_metric_mins.keys():
                    eval_metric_mins[metric] = min(
                        eval_updatelog[metric], eval_metric_mins[metric]
                    )
...
if __name__ == "__main__":
    ...
    # General arguments
    parser.add_argument(
        "--output", default=None, type=str, help="Local output file name or path."
    )
    parser.add_argument(
        "--seed", default=0, type=int, help="Set the seed for the model and training."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--world_model_pred_multilabel_threshold",
        default=0.5,
        type=float,
        help="Probability threshold to predict existence of a sprite in pred_multilabel when prediction_type == 'existence'.",
    )
    parser.add_argument(
        "--world_model_refine_pred_multilabel",
        default=True,
        action="store_true",
        help="Refine pred_multilabel by keeping <=1 of each sprite, exactly 1 avatar, and no entities known to not exist in the level.",
    )
    parser.add_argument(
        "--world_model_no_refine_pred_multilabel",
        dest="world_model_refine_pred_multilabel",
        action="store_false",
        help="Do not refine pred_multilabel.",
    )
    parser.add_argument(
        "--world_model_load_state", default=None, help="Path to world model state dict."
    )
    parser.add_argument(
        "--world_model_key_dim",
        default=256,
        type=int,
        help="World model key embedding dimension.",
    )
    parser.add_argument(
        "--world_model_key_type",
        default="oracle",
        choices=["oracle", "emma", "emma-mlp_scale", "chatgpt"],
        help="What to use to process the descriptors' key tokens.",
    )
    parser.add_argument(
        "--world_model_key_freeze",
        default=False,
        action="store_true",
        help="Whether to freeze key module.",
    )
    parser.add_argument(
        "--world_model_key_unfreeze_step",
        default=5e5,
        type=int,
        help="Train step to unfreeze key module, -1 means never.",
    )
    parser.add_argument(
        "--world_model_val_dim",
        default=256,
        type=int,
        help="World model value embedding dimension.",
    )
    parser.add_argument(
        "--world_model_val_type",
        default="oracle",
        choices=["oracle", "emma", "emma-mlp_scale", "chatgpt", "none"],
        help="What to use to process the descriptors' value tokens.",
    )
    parser.add_argument(
        "--world_model_val_freeze",
        default=False,
        action="store_true",
        help="Whether to freeze val module.",
    )
    parser.add_argument(
        "--world_model_val_unfreeze_step",
        default=5e5,
        type=int,
        help="Train step to unfreeze val module, -1 means never.",
    )
    parser.add_argument(
        "--world_model_latent_size",
        default=512,
        type=int,
        help="World model latent size.",
    )
    parser.add_argument(
        "--world_model_hidden_size",
        default=1024,
        type=int,
        help="World model hidden size.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--world_model_weight_decay",
        default=0,
        type=float,
        help="World model weight decay.",
    )
    parser.add_argument(
        "--world_model_reward_loss_weight",
        default=1,
        type=float,
        help="World model reward loss weight.",
    )
    parser.add_argument(
        "--world_model_done_loss_weight",
        default=1,
        type=float,
        help="World model done loss weight.",
    )
    parser.add_argument(
        "--world_model_loss_source",
        default="real",
        choices=["real", "imag"],
        help="Whether to train on loss of real or imaginary rollouts.",
    )
    parser.add_argument(
        "--world_model_prediction_type",
        default="location",
        choices=["existence", "class", "location"],
        help="What the model predicts.",
    )
    parser.add_argument(
        "--world_model_memory_type",
        default="lstm",
        choices=["mlp", "lstm"],
        help="NN type for memory module of world model.",
    )

    # Dataset arguments
    parser.add_argument(
        "--dataset_path",
        default="custom_dataset/dataset_64x.pickle",
    ...
    parser.add_argument(
        "--train_start_state",
        default="initial",
        choices=["initial", "anywhere"],
        help="Which state that rollouts should start from during training",
    )
    parser.add_argument(
        "--max_rollout_length",
        default=32,
    ...
    parser.add_argument(
        "--batch_size", default=64, type=int, help="batch_size of training input"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Whether to augment the training data by shuffling sprite IDs",
    )
    parser.add_argument(
        "--dropout_prob", default=0.0, type=float, help="probability of dropout layer"
    )
    parser.add_argument(
        "--dropout_loc",
        default="input",
        choices=["sprite", "input", "network", "input,network"],
        help="Where dropout should occur",
    )
    ...
    parser.add_argument(
        "--eval_batch_size", default=1024, type=int, help="batch_size for evaluation"
    )
    parser.add_argument(
        "--n_frames", default=64, type=int, help="number of frames to visualize"
    )
    ...
    if args.world_model_val_type == "oracle":
        args.world_model_val_dim = (
            7  # avatar mvmt type + 3 entity mvmt types + 3 entity role types
        )
