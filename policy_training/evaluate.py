...
from offline_training.chatgpt_groundings.utils import (
...
class PolicyWithWorldModel:
        ...
        world_model_args.update(
            {
                ...
                "done_loss_weight": 0,
                "loss_weights": {"loc": 0, "id": 0, "reward": 0, "done": 0},
            }
        )
        world_model = WorldModel(Namespace(**world_model_args)).to(args.device)
        world_model.load_state_dict(
            torch.load(args.world_model_load_model_from, map_location=args.device)
        )
        world_model.eval()
        return world_model
    ...
    def _simulate_and_evaluate(self, manual, init_grids, init_buffer):
        ...
        h, c = (
            self.world_model.hidden_states.clone(),
            self.world_model.cell_states.clone(),
        )
        for k in range(self.num_simulations // self.batch_size):
            ...
            init_actions = init_actions.unsqueeze(0).repeat(
                self.batch_size // self.num_actions, 1
            )
            init_actions = init_actions.view(-1)
            with torch.no_grad():
                preds = self.world_model.pred(
                    init_grids, self.parsed_manuals, init_actions, sample=True
                )
                ...
                for buffer in sim_buffers:
                    ...
                    with torch.no_grad():
                        a = self.explore_policy(
                            obs_hist, manual, deterministic=False, temperature=self.temp
                        )
                    actions.append(a)
                actions = torch.tensor(actions).to(self.device)
                with torch.no_grad():
                    preds = self.world_model.pred(
                        grids, self.parsed_manuals, actions, sample=True
                    )
                # update buffers, rewards, and dones
                grids = preds["grid"]
                """
            ...
            for i in range(self.batch_size):
                ...
                for j in reversed(range(l)):
                    total_reward = (
                        total_reward * self.discount_factor + sim_rewards[i][j]
                    )
    ...
    def __call__(self, obs, buffer, manual, deterministic=False):
        ...
        with torch.no_grad():
            preds = self.world_model.pred(
                grids, self.parsed_manuals, best_actions, sample=False  # not important
            )
    ...
    for split in splits:
        ...
        for i, episode in enumerate(range(len(split_games[split]))):
            ...
            print(
                ...
                total_reward,
                "avg: ",
                np.average(policy_total_rewards),
            )
            ...
            print(
                ...
                total_reward,
                "avg: ",
                np.average(policy_with_world_model_total_rewards),
            )
        ...
        policy_with_world_model_total_rewards = np.array(
            policy_with_world_model_total_rewards
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--load_state",
        default="stage_1/output/emma_s1_1_train_games_max.pth",
    ...
    parser.add_argument(
        "--world_model_gpt_groundings_path",
        default="../offline_training/chatgpt_groundings/chatgpt_grounding_for_text_all.json",
        type=str,
        help="path to chatgpt groundings",
    )
    parser.add_argument(
        "--world_model_load_model_from",
        default="../offline_training/experiments/gpt_shuffle_balanced_intentions_10k_train_500_eval/ne_nr_or_nm_best_total_loss.ckpt",
    ...
    parser.add_argument(
        "--world_model_attr_embed_dim",
        type=int,
        default=256,
        help="attribute embedding size",
    )
    parser.add_argument(
        "--world_model_action_embed_dim",
        type=int,
        default=256,
        help="action embedding size",
    )
    parser.add_argument(
        "--world_model_desc_key_dim", type=int, default=256, help="description key size"
    )
    parser.add_argument(
        "--world_model_keep_entity_features_for_parsed_manuals", type=int, default=1
    )
    ...
    parser.add_argument(
        ...
        default=5,
        type=float,
        help="temperature of the policy (logits scaling)",
    )
    parser.add_argument(
        "--num_policy_samples",
        default=20,
        type=int,
        help="number of policy samples to evaluate",
    )
    parser.add_argument(
        "--max_lookahead_length", default=5, type=int, help="maximum steps to lookahead"
    )
    parser.add_argument(
        "--max_steps", default=64, type=int, help="max length of an episode"
    )
